...
public class GuestbookModelIndexerWriterContributor implements ModelIndexerWriterContributor<Guestbook> {
    ...
    @Override
    public long getCompanyId(Guestbook baseModel) {
        return baseModel.getCompanyId();
    }

    @Override
    public void modelIndexed(Guestbook baseModel) {
        guestbookEntryBatchReindexer.reindex(baseModel.getGuestbookId(), baseModel.getCompanyId());
    }
}
